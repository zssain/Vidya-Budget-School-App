# Vidya — Build Prompts Book

All 44 build prompts in one file for reading and printing. **To run a prompt, do not paste from this book:** open Claude Code in your project and type `/run <id>`, which loads the same text from `docs/prompts/` together with the project rules.
...
- `views/marks.js`: inputs hold draft values; totals and grades shown in the table come from the last server response (the UI may show "—" for unsaved rows; **no grade maths in JS**). On validation error, highlight the cells from the `cells` param. `views/reportcard.js` renders the DTO; printing uses the prototype's layout for now (P5.2 upgrades).
...
### Goal
Every printed document from HTML templates, printed and saved as PDF using the method in D25, with Hindi printing correctly, matching the layouts schools already use.

### Tasks

#### Task 1 — Template system (`src/print/`)
- One module per document exporting `render(dto, options) -> SafeHtml`; shared `header.js` (logo, name, address, UDISE, phone) and `footer.js` ("Printed on <date> by <name>", small).
- `print.css` page rules: `@page` sizes A4 portrait, A4 landscape, A5, and 80 mm roll (width 80 mm, auto height); margins; `break-inside: avoid` for rows and copies.
- Language option en, hi or both (labels from locale files; values unchanged).
- `printService.print(docType, dto, options)` and `printService.savePdf(...)` implementing D25 for desktop; Android path added in P8.5.
...
#### Task 4 — Settings
Receipt paper, print language and logo upload (already in P3.6) connected to printing. "Test print" button printing the spike page.

#### Task 5 — Tests
- Vitest: each template renders with a sample DTO and escapes names; snapshot of the HTML for receipt A4, A5 and thermal.
...
`undo_field_change(actor, history_id)` (activity.view + settings.edit): writes the losing value back as a new local change with a new HLC, marks `undone_at`. Command `undo_field_change` (**D**). Activity screen shows "Changed on two devices" items with an Undo button.
...
Tests: a table-driven test for every entity × role asserting exact field sets; a teacher never receives `receipt`, `receipt_cancellation`, `user`, fee plan rows, or enrollment fields `rte`, `transport`, `concession`.
...
- Operations generator (seeded): teacher attendance saves and marks edits for own sections, accountant receipts and student field edits, principal concession and cancellations, clock skew, going offline/online, sync in random order.
...
| Read first | AGENTS.md, docs/ARCHITECTURE.md (sections 3 and 5), docs/PLATFORMS.md (Android column), docs/API.md (**M** commands) |
...
### Goal
The same Tauri project builds an Android app that contains only client features, runs on the phone, and shows the mobile sign-in screen.
...
- `src-tauri/Cargo.toml` features: `default = ["desktop-server"]`, `desktop-server = [deps: vidya-server, vidya-backup, vidya-export, tauri-plugin-single-instance, tauri-plugin-autostart ...]`, `mobile-client = [deps: vidya-client]`. Make optional dependencies `optional = true`.
...
- A CI step on ubuntu-latest: `cargo check -p vidya-app --no-default-features --features mobile-client --target aarch64-linux-android` (with NDK set up via a well-known action; read its README) — at minimum `cargo check` with the target, to catch desktop code leaking into mobile.

#### Task 4 — Frontend entry
- Vite: build mode `mobile` (`vite build --mode mobile`) using `src/main-mobile.js`, selected by an env variable in the Tauri Android `beforeBuildCommand`/`beforeDevCommand` (use Tauri's platform-specific configuration file for Android; verify its file name and merge behaviour).
- `main-mobile.js` shows the mobile shell: sign-in screen (username with `@school`, password), bottom tab bar. Nothing else connected yet.
- Viewport meta, safe areas (notch), and large touch targets from UI_GUIDE.md.

#### Task 5 — Size and run
- `npm run android:dev` runs on the connected phone.
- `npm run android:build:apk` for arm64 only (check the split-per-ABI or target option); report the APK size. If over 15 MB, list the largest contributors (`cargo bloat` for the Android target if it works, otherwise explain) and stop for a decision.
...
### Done when (agent)
- [ ] `npm run verify` passes; the Android `cargo check` CI step is green
- [ ] APK size reported
...
- [ ] The APK installs by copying it to the phone (allow "install unknown apps" for your file manager)
...
- Run `scripts/check-i18n.mjs`; make it strict now: any view text not through `t()` fails; any Rust error `message_key` used in code but missing from core locales fails (add a Rust test that scans `crates/**/src/**/*.rs` for `"...error..."` keys used with `DomainError`/`ServiceError` constructors and checks both locale files).
- Fix every gap.

#### Task 2 — Draft translations
- Translate every key in `src/locales/hi.json` and `crates/vidya-core/locales/hi.json`.
- Style: simple school Hindi as spoken in offices (उपस्थिति, फ़ीस, रसीद, अंक, कक्षा, छात्र, अभिभावक, शिक्षक, प्रधानाचार्य, बकाया, जमा, रद्द करें, बैकअप). Avoid heavy Sanskritised words. Keep English where schools normally use it (UPI, Aadhaar, APAAR, UDISE, RTE, PDF, Excel, Wi-Fi).
...
- Print language setting en/hi/both applied to all templates; "both" shows "Receipt / रसीद" style labels.
- Excel exports: headers follow the user's language.

#### Task 6 — Installers
NSIS installer: add Hindi language (verify Tauri's NSIS language option names). macOS: add `hi` localisation for the app name and Info.plist descriptions (verify how Tauri supports localised plist strings; if not supported, record in KNOWN_ISSUES).
