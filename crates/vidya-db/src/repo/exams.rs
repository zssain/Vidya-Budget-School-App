...
/// All exams of a session, active or not, ordered by `sort_order`.
pub fn list_all(conn: &Connection, session_id: &str) -> Result<Vec<ExamRow>, DbError> {
    let mut stmt = conn.prepare(
        "SELECT id, session_id, name, max_marks, sort_order, active FROM exams
...
/// Renames/reorders/reactivates an exam and sets its maximum.
pub fn update(
    tx: &Transaction<'_>,
    id: &str,
    name: &str,
    ...
    tx.execute(
        "UPDATE exams SET name = ?2, max_marks = ?3, sort_order = ?4, active = ?5, updated_hlc = ?6 WHERE id = ?1",
        rusqlite::params![id, name, max_marks, sort_order, i64::from(active), hlc],
    )?;
    Ok(())
}
