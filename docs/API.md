...
|---|---|---|---|
| `app_status` | no session | → `AppStatusDto { ready, failure?, hasSchool, licensed, serverAllowed, schoolName?, schoolCode?, platform, version }` | First call on start. `ready=false` with `failure` (one of `DataFolder`, `SecureStorage`, `WrongKey`, `Migration`, `Unknown`) drives the startup error screen. `serverAllowed` stays `false` until P4.2; `deviceLocked`/`wizardStep` are added in P4.2 |
