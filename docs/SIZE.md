...
| 2026-09-21 | P2.7 | Real command layer wires sign-in through `vidya-services`, so argon2/sha2/uuid/tracing now link into the shipped binary (as anticipated at P2.5). `sha2` 0.10 added (P2.6). `load_sample_school`/`vidya-testkit` are debug-only (stripped from release). | pending new CI | pending VM | 2.19 MB (2,188,294 bytes; +114,537 vs P2.6) | 3.84 MB (3752 KiB on disk) | stale P1.3 | stale P1.3 | stale P1.3 | stale P1.3 | pending P8.1 |
